...
public class Driver {
    public static void main(String[] args) {
        ...
        int[][][] data = And.andData;
        double[] weights = And.INITIAL_WEIGHTS;
        ...
        while(errorFlag) {
           ...
           for(int x=0; x< data.length; x++){
               double weightedSum= and.calculateWeightedSum(data[x][0],weights);
               int result= and.applyActivationFunction(weightedSum);
               error=data[x][1][0]-result;
               if(error !=0) errorFlag=true;
               adjustedWeights= and.adjustedWeights(data[x][0], weights, error);
         ...
         public void printHeading(int epochNumber){
            System.out.println("\n=========================================Epoch "+epochNumber+"===================================================");
            System.out.println("  w1  |  w2  | x1 | x2 | Target Result  |  Result       |     error        |  Weighted SUm   | Adjusted w1 |   djusted w2");
            System.out.println("----------------------------------------------------------------------------------------------------------------------");
